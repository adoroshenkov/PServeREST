    ...
    try {
        while ($listener.IsListening){
            ...
            try {
                ...
                # Kill cached function from dot-sourced file "Resorce.ps1" (for reason when "Resorce.ps1" file has been modified while, but old $ResourceHandlerProc is still in memory and can execute)
                Get-ChildItem function: | Where-Object {($_.ScriptBlock.File) -and (($_.ScriptBlock.File.Split("\")[-1]) -eq "$Resource.ps1")} | Remove-Item
